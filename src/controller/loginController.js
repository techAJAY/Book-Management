...
const jwt = require("jsonwebtoken");
 ...
 return Object.keys(requestBody).length > 0
    //It will return an array of all keys. so, we can simply get the length of an array with .length

         }
            

       //api to login in user
        ...
        const loginUser = async function(req, res) {
                try {
                    ...
                    if (requestBody.email && requestBody.password) {
                        ...
                        let token = jwt.sign(payload, 'projectthree',{ expiresIn: '1500000s' })   
